...
/**
 * 產生業務代號：依照使用者選的支出項目帶 W/X/Y/Z，再加流水號
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $expenseItemOriginal = $_POST['支出項目'] ?? '';  // 前端傳來的值：活動費用、獎學金、經濟扶助、其他

    // ★將支出項目加上對應前綴後再存入資料庫
    switch ($expenseItemOriginal) {
        case '活動費用':
            $expenseItemStored = 'W活動費用';
            break;
        case '獎學金':
            $expenseItemStored = 'X獎學金';
            break;
        case '經濟扶助':
            $expenseItemStored = 'Y經濟扶助';
            break;
        case '其他':
            $expenseItemStored = 'Z其他';
            break;
        default:
            // 以防萬一
            $expenseItemStored = $expenseItemOriginal;
            break;
    }
    ...
    try {
        ...
        if ($stmtCheck->fetchColumn() == 0) {
            $stmt1 = $pdo->prepare("
                ...
                VALUES (?, ?, ?, ?)
            ");
            $stmt1->execute([$recipientCode, $recipientCode, $phone, $address]);
        }
        ...
        echo "表單提交成功 | 受款人代號: {$recipientCode}, 經辦代號: {$businessCode}, 交易單號: {$trxID}";
        exit;
    } catch (Exception $ex) {
        $pdo->rollBack();
