...
try {
    ...
    $pdo->exec("
        CREATE TABLE IF NOT EXISTS 經辦人交易檔 (
            ...
            實支金額    DECIMAL(10,2) DEFAULT 0.00,
            結餘        DECIMAL(10,2) DEFAULT 0.00,
            審核狀態    VARCHAR(10)  DEFAULT '待審核',
            PRIMARY KEY (交易單號),
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $expenseItemOriginal = $_POST['支出項目'] ?? '';
    $desc                = $_POST['說明'] ?? '';
    ...
    try {
        ...
        $stmt3->execute([
            ...
            $recipientCode,
            $businessCode,
            $amtFormatted,       // 存入已格式化的金額字串 (例如 "33,333.00")
            $amtChinese ?: null, // 中文金額（前端提供）
            date('Y-m-d H:i:s'),
            $payMethod,
            $bank ?: null,
            $branch ?: null,
