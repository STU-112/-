...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box;}
    ...
    .search-box input{padding:6px 8px;border:1px solid #ccc;border-radius:4px;}
    ...
    .record-table td:first-child{width:40px;}
  </style>
</head>
<body>
  <div class="container">
    ...
    <div class="search-box">
      <label>關鍵字篩選：<input type="text" id="searchInput" placeholder="輸入關鍵字，以空格或逗號分隔"></label>
    </div>
        ...
        <tbody>
          <?php foreach ($detailRows as $d): ?>
            ...
            <tr class="summary-row" data-sub="<?= htmlspecialchars($sub) ?>">
              <td>
                <input type="checkbox" class="summary-select" data-sub="<?= htmlspecialchars($sub) ?>" checked>
              </td>
            ...
            <tr class="full-section" data-parent="<?= htmlspecialchars($sub) ?>">
              <td colspan="4">
                <table class="record-table" data-parent="<?= htmlspecialchars($sub) ?>">
                  <thead>
                    <tr>
                      <th></th>
                      <th>業務代號</th>
                  ...
                  <tbody>
                    <?php foreach ($fullMap[$sub] as $r): ?>
                      <tr>
                        <td>
                          <input type="checkbox" class="row-select" data-parent="<?= htmlspecialchars($sub) ?>" checked>
                        </td>
  ...
  <div style="text-align:center;margin:20px;">
    <a href="javascript:histfory.back()" style="color:#2a7f7d;text-decoration:none;font-weight:600;">
      ← 回上一頁
    </a>
  </div>
  ...
  <script>
    ...
    document.getElementById('searchInput').addEventListener('input', function() {
      ...
      // 3. 先過濾明細列：任一關鍵字符合就顯示，否則隱藏
      document.querySelectorAll('.record-table tbody tr').forEach(tr => {
        const txt = tr.textContent.toLowerCase();
        const keep = terms.some(t => txt.includes(t));
        tr.style.display = keep ? '' : 'none';
      });

      // 4. 根據明細結果，顯示／隱藏對應的 summary-row 及 full-section
      document.querySelectorAll('.summary-row').forEach(summary => {
        const sub = summary.dataset.sub;
        const rows = document.querySelectorAll(`.record-table[data-parent="${sub}"] tbody tr`);
        const anyVisible = Array.from(rows).some(r => r.style.display === '');
        summary.style.display = anyVisible ? '' : 'none';
        const fullSec = document.querySelector(`.full-section[data-parent="${sub}"]`);
        fullSec.style.display = anyVisible ? 'table-row' : 'none';
      });
    });
