...
// 查詢金額小於1000的資料 
$sql = "SELECT serial_count, form_type, amount, fillDate, recipient FROM pay_table WHERE amount >=50001"; 
...
if ($result && $result->num_rows > 0) { 
    ...
    <script>
        ...
        function controlButtons(amount, userDepartment, department, approveBtn, rejectBtn) {
            if (amount <= 1000 && userDepartment === department) {
                // 金額 <= 1000 且部門相同，所有按鈕可用
                approveBtn.disabled = false;
                rejectBtn.disabled = false;
            } else if (amount > 1000 && userDepartment !== department) {
                // 金額 > 1000 且部門不同，只有上呈按鈕可用
                approveBtn.disabled = false;
                rejectBtn.disabled = true;
            } else if (amount > 1000) {
                // 金額 > 1000 且部門相同，只有上呈按鈕可用
                approveBtn.disabled = false;
                rejectBtn.disabled = true;
            } else if (amount <= 1000 && userDepartment !== department) {
                // 金額 <= 1000 且部門不同，允許通過和不通過
                approveBtn.disabled = false;
                rejectBtn.disabled = false;
            }
        }
        ...
        function approveAction(amount, userDepartment, department) {
            if (amount <= 1000 && userDepartment === department) {
                alert('權限內，金額低於 1000 通過');
            } else if (amount > 1000 && userDepartment !== department) {
                alert('權限過大，跨部門上呈');
            } else if (amount > 1000) {
                alert('權限過大上呈');
            } else if (amount < 1000 && userDepartment !== department) {
                alert('低於 1000 塊以下，跨部門上呈');
            }
        }
