...
function generateSerialNumber($連接) {
    ...
    $prefix = "B{$year}{$month}";
    $sql = "SELECT COUNT(*) as count FROM 支出報帳表單 WHERE count LIKE '$prefix%'";
...
if (!empty($_FILES['image-upload']['name'])) {
   ...
   $insert_record_sql = "INSERT INTO 支出報帳表單 (
    count, 受款人, 填表日期, 付款日期, 支出項目, 活動名稱, 專案日期, 獎學金人數,
    專案名稱, 主題, 獎學金日期, 經濟扶助, 其他項目, 說明, 支付方式, 國字金額_hidden,
    金額, 簽收金額, 簽收人, 簽收日, 銀行郵局, 分行, 戶名, 帳戶, 票號, 到期日,
    預收金額, csv_file_path, image_file_path
) VALUES (
    '$流水號', '$受款人', '$填表日期', $付款日期, '$支出項目', $活動名稱, $專案日期,
    $獎學金人數, $專案名稱, $主題, $獎學金日期, $經濟扶助, '$其他項目', '$說明',
    '$支付方式', '$國字金額', $金額, $簽收金額, $簽收人, $簽收日, $銀行郵局, $分行,
    $戶名, $帳戶, $票號, $到期日, $預收金額, '$csv_file_path', '$image_file_path'
)";
