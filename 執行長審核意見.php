...
if (mysqli_query($db_link, $insert_record_sql)) {
    echo "<p style='color: green;'>記錄已成功提交！3 秒後將返回執行長頁面。</p>";
    echo "<script>setTimeout(function(){ window.location.href = '執行長.php'; }, 3000);</script>";
} else {
    if (mysqli_errno($db_link) == 1062) {
        // 1062 是重複鍵的錯誤代碼
        echo "<p style='color: orange;'>插入失敗：該單號已存在。3 秒後將返回執行長頁面。</p>";
        echo "<script>setTimeout(function(){ window.location.href = '執行長.php'; }, 3000);</script>";
    } else {
