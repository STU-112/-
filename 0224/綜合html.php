...
<body>
  ...
  <div class="container">
    <div class="form-container" aria-labelledby="formTitle">
      <!-- 新增: enctype="multipart/form-data" 以支援檔案上傳 -->
      <form id="paymentForm" action="綜合.php" method="POST" enctype="multipart/form-data" novalidate>
        <h1>財團法人台北市失親兒福利基金會</h1>

        <!-- 選擇表單 -->
        <div class="form-group">
          <label for="選擇表單">選擇表單：<span class="required-star">*</span></label>
          <select id="選擇表單" name="選擇表單" onchange="toggleFormSections()" required>
        ...
        <div id="支出核銷區域" class="hidden">
          <div class="form-group">
              ...
              <button type="submit" id="submitBtn">搜尋</button>
          </div>
        </div>
        ...
        <div id="支出報帳區域" class="hidden">
            ...
            <div class="form-group">
              ...
              <input type="number" id="國字金額" name="金額" placeholder="請輸入金額" min="0" required aria-required="true" oninput="convertAmountToChinese()">
            ...
            <div id="現金簽收欄位" class="conditional-group" style="display: none;">
              ...
              <div class="form-group">
                <label for="簽收人">簽收人：<span class="required-star">*</span></label>
                <input type="text" id="簽收人" name="簽收人" placeholder="請輸入簽收人" required>
            ...
            <div id="支票欄位" class="conditional-group" style="display: none;" aria-live="polite">
              ...
              <div class="form-group">
                ...
                <input type="number" id="預支金額" name="結餘繳回" min="0" required>
            ...
            <!-- ================================ -->
            <div class="form-group">
              <label for="單據張數">單據張數：<span class="required-star">*</span></label>
              <input type="number" id="單據張數" name="單據張數" min="1" required>
              <span class="error-message" id="單據張數-error"></span>
            </div>

            <div class="form-group">
              <label for="image_file">上傳圖片：</label>
              <input type="file" id="image_file" name="image_file" accept="image/*">
              <span class="error-message" id="image_file-error"></span>
            </div>

            <div class="form-group">
              <label for="csv_file">上傳CSV檔案：</label>
              <input type="file" id="csv_file" name="csv_file" accept=".csv">
              <span class="error-message" id="csv_file-error"></span>
            </div>
            <!-- 新增結束 -->
  ...
  <script>
    function toggleFormSections() {
      ...
      const 支出核銷區域 = document.getElementById('支出核銷區域');
      const 支出報帳區域 = document.getElementById('支出報帳區域');

      // 顯示對應的表單區域，隱藏其他區域
      支出核銷區域.classList.toggle('hidden', formSelection !== '支出核銷');
      支出報帳區域.classList.toggle('hidden', formSelection !== '支出報帳');
    };
...
function validateForm() {
    ...
    requiredFields.forEach(field => {
        ...
        if (!field.value.trim()) {
            isValid = false;
            errorElement.textContent = "此欄位不能空白。";
