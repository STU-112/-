...
$create_table_sql = "CREATE TABLE IF NOT EXISTS 註冊資料表 (
    ...
    PRIMARY KEY (員工編號),
FOREIGN KEY (部門) REFERENCES 部門(部門代碼),
    FOREIGN KEY (職位) REFERENCES 職位(職位代碼)
)";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $員工編號 = mysqli_real_escape_string($db_link, $_POST['員工編號']);
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        // 插入記錄
        $insert_record_sql = "INSERT INTO 註冊資料表 (員工編號,姓名, 電話, 地址, 部門, 職位, 帳號, 密碼, 權限管理)
                              VALUES ('$員工編號','$name', '$phone', '$address', '$department', '$position', '$username', '$password', '$permission')";
