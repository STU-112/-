...
$單號選項 = "";
$sql_單號 = "SELECT 交易單號,受款人代號 FROM 經辦人交易檔 
              WHERE 交易單號 LIKE 'A%' 
              AND 審核狀態 IN ('核銷未通過', '預支完成審查') 
              ORDER BY 交易單號 DESC";
$result = $連接->query($sql_單號);
if ($result && $result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
    $交易單號 = htmlspecialchars($row['交易單號']);
    $受款人代號 = htmlspecialchars($row['受款人代號']);
    $顯示文字 = $交易單號 . " - " . $受款人代號;
    $單號選項 .= "<option value=\"$交易單號\">$顯示文字</option>\n";
}
...
<head>
  ...
  <style>
    ...
    * {
    ...
    #recipientName { color: red; font-weight: bold; margin-right: 8px; user-select: none; }
...
<body>
  <div class="container">
	...
	<!-- ✅ 這是 "查詢單號" 的獨立表單 (search.php) -->
	      <div id="formSections" class="hidden"> 
        <form id="searchForm" action="searchhtml7.php" method="POST">
		  <div id="支出核銷區域" class="hidden">
            <div class="form-group">
                <label for="單號查詢">單號查詢：</label>
				<select id="單號查詢" name="單號查詢" required>
					<option value="">請選擇單號</option>
				<?php echo $單號選項; ?>
				</select>

				
               <!-- <input type="text" id="單號查詢" name="單號查詢" placeholder="請輸入單號" required> -->
            </div>
            <div class="btn-row">
                <button type="submit" id="submitBtnsearch">搜尋</button>
            </div>
        </form>
	   </div>	 
	   </div>
...
<!-- 受款人：代號 (Rxx) + 姓名 -->
      <div class="form-group">
        ...
        <div id="recipientRow">
          ...
          <!-- 受款人（含自動產生編號），調整後只顯示並傳送受款人代號 
          <div class="form-group">
            <label for="受款人">受款人：</label>
    ...
    function togglePaymentFields(){
      ...
      allFields.forEach(id=>{
        ...
        Array.from(section.querySelectorAll('input')).forEach(ctrl=>{
          ctrl.removeAttribute('required'); ctrl.value='';
        });
      });
      if(payWay==='現金'){
