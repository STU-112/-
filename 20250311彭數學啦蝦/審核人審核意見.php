...
// $職位編號查詢 = "SELECT * FROM 職位";
// $結果 = $db_link_註冊->query($職位編號查詢);
// while ($row = $結果->fetch_assoc()) {
    // echo "編號: " . $row["編號"] . " - 職位名稱: " . $row["職位名稱"] . "<br>";
// }
// if ($結果 && $結果->num_rows > 0) {
    // $row = $結果->fetch_assoc();
    // $當前職位編號 = $row["編號"];
    // echo "找到職位編號: " . $當前職位編號 . "<br>";
// } else {
    // echo "找不到對應的職位編號！請檢查 `職位` 資料表的內容。<br>";
// }
...
$create_table_sql = "CREATE TABLE IF NOT EXISTS 審核意見 (
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $insert_record_sql = "INSERT INTO 督導審核意見 (單號, 審核意見, 狀態, 審核人, 職位名稱, 審核時間) 
