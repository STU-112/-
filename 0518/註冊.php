...
// 創建資料表（如果表不存在）
$create_table_sql = "CREATE TABLE IF NOT EXISTS 使用者帳密 (
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $emp_id = mysqli_real_escape_string($db_link, $_POST['員工編號']);
    ...
    // 檢查帳號是否已存在
    $check_user_sql = "SELECT 帳號 FROM 使用者帳密 WHERE 帳號 = '$username'";
    $result = mysqli_query($db_link, $check_user_sql);
    if (mysqli_num_rows($result) > 0) {
        echo "帳號已存在，請使用其他帳號註冊。<br>";
    } else {
        // 插入記錄
        $insert_record_sql = "INSERT INTO 使用者帳密 (員工編號, 姓名, 電話, 地址, 部門, 職位, 帳號, 密碼, 權限管理)
                              VALUES ('$emp_id', '$name', '$phone', '$address', '$department', '$position', '$username', '$password', '$permission')";
        ...
        if (mysqli_query($db_link, $insert_record_sql)) {
            ...
            <body>
                <div class="message-container">
                    <h1>註冊成功！</h1>
                    <p>您將在 <span id="countdown" class="countdown">3</span> 秒內自動跳轉到登入頁面。</p>
                </div>
                <script>
                    ...
                    var interval = setInterval(function() {
                        ...
                        if (seconds <= 0) {
                            ...
                            window.location.href = "系統管理員.php";
                        }
                    }, 1000);
                </script>
            </body>
