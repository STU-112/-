...
// 創建資料表（名稱改為報帳）
$create_table_sql = "CREATE TABLE IF NOT EXISTS `報帳` (
...
if (mysqli_query($連接, $create_table_sql)) {
    // echo "報帳資料表創建成功或已存在!!<br>";
} else {
    die("創建報帳資料表失敗: " . mysqli_error($連接) . "<br>");
}
...
function generateSerialNumber($連接) {
    ...
    $prefix = "B{$year}{$month}"; // 前綴，如 B11311

    // 查詢當前月份的最大流水號
    $sql = "SELECT MAX(`count`) AS max_count FROM `報帳` WHERE `count` LIKE '{$prefix}%'";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        // 插入資料
        $insert_record_sql = "INSERT INTO `報帳`
            (`count`, 受款人, 填表日期, 付款日期, 支出項目, 活動名稱, 專案日期, 獎學金人數, 專案名稱, 主題, 獎學金日期, 經濟扶助, 其他項目, 說明, 支付方式, 國字金額, 國字金額_hidden, 金額, 簽收金額, 簽收人, 簽收日, 銀行郵局, 分行, 戶名, 帳戶, 票號, 到期日, 預收金額)
            VALUES
            ('$流水號', '$受款人', '$填表日期', $付款日期, '$支出項目', $活動名稱, $專案日期, $獎學金人數, $專案名稱, $主題, $獎學金日期, $經濟扶助, $其他項目, '$說明', '$支付方式', '$國字金額', '$國字金額_hidden', $金額, $簽收金額, $簽收人, $簽收日, $銀行郵局, $分行, $戶名, $帳戶, $票號, $到期日, $預收金額)";
