...
function generateSerialNumber($連接) {
    ...
    $prefix = "{$year}{$month}";
    $sql = "SELECT COUNT(*) as count FROM 報帳 WHERE 單號 LIKE 'B{$prefix}%'";
    $result = mysqli_query($連接, $sql);
    if (!$result) {
        die("查詢流水號失敗: " . mysqli_error($連接) . "<br>");
    }
    $row = mysqli_fetch_assoc($result);
    $serialNumber = 'B' . $prefix . str_pad($row['count'] + 1, 5, '0', STR_PAD_LEFT);
    return $serialNumber;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $insert_record_sql = "INSERT INTO 報帳 (單號, 受款人, 填表日期, 付款日期, 支出項目, 活動名稱, 專案日期, 獎學金人數, 專案名稱, 主題, 獎學金日期, 經濟扶助, 其他項目, 說明, 支付方式, 國字金額, 國字金額_hidden, 金額, 簽收金額, 簽收人, 簽收日, 銀行郵局, 分行, 戶名, 帳戶, 票號, 到期日, 預收金額)
    VALUES ('$流水號', '$受款人', '$填表日期', $付款日期, '$支出項目', $活動名稱, $專案日期, $獎學金人數, $專案名稱, $主題, $獎學金日期, $經濟扶助, '$其他項目', '$說明', '$支付方式', '$國字金額', '$國字金額_hidden', $金額, $簽收金額, $簽收人, $簽收日, $銀行郵局, $分行, $戶名, $帳戶, $票號, $到期日, $預收金額)";
